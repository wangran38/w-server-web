<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="评估员头像" width="110px" align="center">
        <template slot-scope="{ row }">
          <el-avatar :src="row.avatar"></el-avatar>
        </template>
      </el-table-column>
      <el-table-column label="评估名称" width="110px" align="center">
        <template slot-scope="{ row }">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      ...
      >
        ...
        <el-form-item label="用户头像" prop="avatar">
          ...
          >
            <img
              v-if="this.temp.avatar"
              :src="this.temp.avatar"
              class="avatar"
            />
...
<script>
import { getlist, add } from "@/api/assessors/assessors";
  ...
  methods: {
    ...
    updateData() {
      this.$refs["dataForm"].validate((valid) => {
        if (valid) {
          //处理父级ID
          if (typeof this.temp.gid == "undefined") {
            //判断是否为空
            this.temp.gid = 0;
            // console.log(this.temp.pid)
          } else {
            if (Array.isArray(this.temp.gid) == true) {
      ...
      this.$confirm("确定要删除【" + row.username + "】吗？", "提示", {
        ...
        deladmin(row).then((response) => {
          ...
          if (response.code == 200) {
            ...
            this.list.splice(index, 1); //当前页面的数据-1
          } else {
